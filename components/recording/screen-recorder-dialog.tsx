 ...
 const [uploading, setUploading] = useState(false);
      ...
      if (url) {
        setUploadedUrl(url);
         setVideoUrl(url);        // 👈 save to zustand
          onOpenChange(false);   // ✅ close dialog BEFORE navigating
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {/* ✅ max-h + overflow-y-auto so preview never pushes buttons off screen */}
      <DialogContent className="sm:max-w-2xl max-h-full  z-50 border-border/60 bg-card/95 backdrop-blur-sm p-0">

        {/* Sticky header */}
        <div className="sticky top-0  bg-card/95 backdrop-blur-sm px-6 py-3 border-b border-border/60">
        ...
        <div className="px-6 py-3 space-y-2">
          ...
          {previewUrl && (
            <div className="space-y-3">
              ...
              {/* ✅ max-h-64 caps video height so buttons stay reachable */}
              <video
                src={previewUrl}
                controls
                className="w-full max-h-64 rounded-xl border border-border/60 bg-black object-contain"
              />
              ...
              <div className="flex justify-end gap-2.5 pt-1">
                ...
                <Button
                  ...
                  className="gap-2 bg-orange-500 hover:bg-orange-400 text-white font-semibold shadow-[0_0_12px_rgba(249,138,36,0.3)] hover:shadow-[0_0_18px_rgba(249,138,36,0.5)] transition-all duration-200 px-5 disabled:opacity-70 disabled:cursor-not-allowed"
                >
                  {uploading ? (
                    <>
